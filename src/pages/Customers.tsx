...
import { Search, UserCircle, Package, MapPin, ShoppingBag, Loader2, ArrowUpDown } from 'lucide-react';
...
const Customers = () => {
    ...
    useEffect(() => {
        const loadCustomers = async () => {
            try {
                setLoading(true);
                const data = await fetchCustomers(page, 12, search); // Increased limit for full-screen
                                        ...
                                        <tr key={i} className="hover:bg-gray-50 transition-colors group">
                                            <td className="py-4 px-6">
                                                <div className="flex items-center gap-4">
                                                    ...
                                                    <div>
                                                        <p className="font-bold text-gray-900 group-hover:text-[#0a2e2a]">{p.name}</p>
                                                        <p className="text-[10px] text-gray-400 font-mono">{p.patient_id}</p>
                                                    </div>
                                                </div>
                                            </td>
